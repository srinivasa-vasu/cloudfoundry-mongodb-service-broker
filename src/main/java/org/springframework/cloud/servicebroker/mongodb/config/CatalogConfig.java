...
public class CatalogConfig {
	...
	public enum ServicePlan {
		...
		// @formatter:off
		DEFAULT("default", "default", "This is a default free plan", true, true, getPlanMetadata(D)), 
        GOLD("gold", "gold", "This is a paid gold plan", false, true, getPlanMetadata(G)), 
        PLATINUM("platinum", "platinum", "This is a paid premium plan", false, true, getPlanMetadata(P));
        // @formatter:on

		ServicePlan(String id, String name, String description, boolean free,
				boolean bindable, Map<String, Object> metadata) {
			this.id = id;
			this.name = name;
		...
		private static Map<String, Object> getPlanMetadata(ServicePlanIdentifier plan) {
			...
			case D:
				...
				bulletsList = Arrays.asList("128Mi Storage (enforced)",
						"Single instance");
				break;
			case G:
				costsMap.put(AMOUNT, new HashMap<String, Object>() {
	...
	public Catalog catalog() {
		...
		for (ServicePlan plan : ServicePlan.values()) {
			planList.add(Plan.builder().id(serviceId + plan.getId()).name(plan.getName())
					.description(plan.getDescription()).metadata(plan.getMetadata())
					.free(plan.isFree()).bindable(plan.isBindable()).build());
		}

		return Catalog.builder().serviceDefinitions(Collections.singletonList(ServiceDefinition
				.builder().id(serviceId).name(serviceId)
				.description("A PKS MongoDB on-demand service broker implementation")
				.bindable(true).plans(planList).tags(Arrays.asList("mongodb", "document"))
				.metadata(getServiceDefinitionMetadata()).build())).build();
	}
