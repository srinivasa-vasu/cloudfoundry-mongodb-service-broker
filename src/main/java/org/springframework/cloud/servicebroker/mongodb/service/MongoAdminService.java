...
public class MongoAdminService {
	...
	MongoDatabase createDatabase(String databaseName) throws MongoServiceException {
		try {
			...
			// TODO: figure out how to clean the db in a different "transaction" so the
			// create is flushed
			// currently dropping the column is preventing the database from being
			// created.
			// col.drop();
	...
	private void runCommand(MongoDatabase db, Map<String, Object> commandArguments) {
	...
	private void addDbOwnerRole(String databaseName) {
		...
		commandArguments.put("roles", Collections.singletonList(roles));
		runCommand(db, commandArguments);
	}

	void createUser(String database, String username, String password)
